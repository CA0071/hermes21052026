export default {
  title: "Set Up Your AI Provider",
  subtitle: "Choose a provider and configure it to get started",
  providerCards: {
    openrouter: { name: "OpenRouter", desc: "200+ models", tag: "Recommended" },
    anthropic: { name: "Anthropic", desc: "Claude models", tag: "" },
    openai: { name: "OpenAI", desc: "GPT models", tag: "" },
    local: {
      name: "Local / OpenAI-Compatible",
      desc: "LM Studio, Ollama, Groq, DeepSeek, Together…",
      tag: "",
    },
  },
  localPresets: {
    lmstudio: "LM Studio",
    ollama: "Ollama",
    vllm: "vLLM",
    llamacpp: "llama.cpp",
    groq: "Groq",
    deepseek: "DeepSeek",
    together: "Together AI",
    fireworks: "Fireworks",
    cerebras: "Cerebras",
    mistral: "Mistral",
  },
  serverPreset: "Server Preset",
  localGroupLabel: "Local Servers",
  remoteGroupLabel: "Remote OpenAI-Compatible APIs",
  serverUrl: "Base URL",
  modelName: "Model Name",
  localServerHint: "Make sure your local server is running before continuing",
  customServerHint: "Pick a preset or paste any OpenAI-compatible base URL",
  customApiKeyLabel: "API Key",
  customApiKeyHint: "Required for remote APIs. Leave blank for localhost.",
  defaultModelHint: "Leave blank to use the server's default model",
  missingApiKey: "Please enter an API key",
  missingServerUrl: "Please enter the server URL",
  saveFailed: "Failed to save configuration",
  noKeyHint: "Don't have a key? Get one here",
  noApiKeyTitle: "No API key needed",
  noApiKeyHint: "This provider uses its own account login.",
  codexAuthTitle: "Use your ChatGPT subscription",
  codexAuthHint:
    "Sign in with ChatGPT in your browser. Hermes stores Codex auth separately from OpenAI API keys.",
  codexAuthCommand: "hermes login --provider openai-codex",
  codexLoginButton: "Sign in with ChatGPT",
  codexLoginButtonRunning: "Waiting for sign-in...",
  codexLoginButtonSignedIn: "Signed in",
  codexOpenBrowser: "Open browser",
  codexUserCodeLabel: "Sign-in code",
  codexCheckingAuth: "Checking Codex sign-in status...",
  codexNotSignedIn: "Sign in to continue with your ChatGPT subscription.",
  codexLoginStarting: "Starting sign-in...",
  codexLoginWaiting: "Browser opened. Enter the code, then finish sign-in.",
  codexLoginSuccess: "Signed in. You can continue.",
  codexLoginFailed: "Sign-in failed",
  codexLoginCancelled: "Sign-in cancelled.",
  continue: "Continue",
  saving: "Saving...",
  apiKeyLabel: "{{provider}} API Key",
  localNoKeyNeeded: "No API key needed",
  localLlm: "Local LLM",
  modelBaseUrlPlaceholder: "http://localhost:1234/v1",
  modelNamePlaceholder: "e.g. llama-3.1-8b",
} as const;
