export default {
  title: "Configure seu Provedor de IA",
  subtitle: "Escolha um provedor e configure-o para começar",
  providerCards: {
    openrouter: {
      name: "OpenRouter",
      desc: "Mais de 200 modelos",
      tag: "Recomendado",
    },
    anthropic: { name: "Anthropic", desc: "Modelos Claude", tag: "" },
    openai: { name: "OpenAI", desc: "Modelos GPT", tag: "" },
    local: {
      name: "Local / Compatível com OpenAI",
      desc: "LM Studio, Ollama, Groq, DeepSeek, Together…",
      tag: "",
    },
  },
  localPresets: {
    lmstudio: "LM Studio",
    ollama: "Ollama",
    vllm: "vLLM",
    llamacpp: "llama.cpp",
    groq: "Groq",
    deepseek: "DeepSeek",
    together: "Together AI",
    fireworks: "Fireworks",
    cerebras: "Cerebras",
    mistral: "Mistral",
  },
  serverPreset: "Preset do Servidor",
  localGroupLabel: "Servidores Locais",
  remoteGroupLabel: "APIs Remotas Compatíveis com OpenAI",
  serverUrl: "URL Base",
  modelName: "Nome do Modelo",
  localServerHint:
    "Certifique-se de que seu servidor local está rodando antes de continuar",
  customServerHint:
    "Escolha um preset ou cole qualquer URL base compatível com OpenAI",
  customApiKeyLabel: "Chave da API",
  customApiKeyHint:
    "Obrigatório para APIs remotas. Deixe em branco para localhost.",
  defaultModelHint: "Deixe em branco para usar o modelo padrão do servidor",
  missingApiKey: "Por favor, insira uma chave de API",
  missingServerUrl: "Por favor, insira a URL do servidor",
  saveFailed: "Falha ao salvar a configuração",
  noKeyHint: "Não tem uma chave? Consiga uma aqui",
  noApiKeyTitle: "Nenhuma chave de API necessaria",
  noApiKeyHint: "Este provedor usa seu proprio login de conta.",
  codexAuthTitle: "Use sua assinatura do ChatGPT",
  codexAuthHint:
    "Entre com o ChatGPT no navegador. O Hermes salva a autenticacao do Codex separada das chaves de API da OpenAI.",
  codexAuthCommand: "hermes login --provider openai-codex",
  codexLoginButton: "Entrar com ChatGPT",
  codexLoginButtonRunning: "Aguardando login...",
  codexLoginButtonSignedIn: "Conectado",
  codexOpenBrowser: "Abrir navegador",
  codexUserCodeLabel: "Codigo de login",
  codexCheckingAuth: "Verificando login do Codex...",
  codexNotSignedIn: "Entre para continuar com sua assinatura do ChatGPT.",
  codexLoginStarting: "Iniciando login...",
  codexLoginWaiting: "Navegador aberto. Digite o codigo e conclua o login.",
  codexLoginSuccess: "Conectado. Voce pode continuar.",
  codexLoginFailed: "Falha no login",
  codexLoginCancelled: "Login cancelado.",
  continue: "Continuar",
  saving: "Salvando...",
  apiKeyLabel: "Chave da API {{provider}}",
  localNoKeyNeeded: "Nenhuma chave de API necessária",
  localLlm: "LLM Local",
  modelBaseUrlPlaceholder: "http://localhost:1234/v1",
  modelNamePlaceholder: "ex: llama-3.1-8b",
} as const;
